...
def evaluate(model, loader, criterion, device):
    ...
    cm = confusion_matrix(trues, preds, labels=[0, 1])  # Pastikan bentuk confusion matrix tetap 2x2 meski hanya satu kelas muncul.

    avg_loss = total_loss / len(loader) if len(loader) > 0 else 0.0  # Hindari pembagian dengan nol saat loader kosong.
    return avg_loss, acc, p, r, f1, cm, preds, trues


def plot_confusion_matrix(cm, class_names=None, save_path=None):
    if class_names is None:
        class_names = ['NON-SELF-HARM', 'SELF-HARM']  # Hindari default argument yang bersifat mutable.
...
def analyze_model_parameters(model):
    ...
    if hasattr(model.clip, 'text_model'):
        ...
        print("CLIP Text Encoder:   0 params | Trainable: 0 (VISION-ONLY CLIP)")
    ...
    # Periksa apakah model memakai Transformer untuk proses fusi.
    ...
    # Periksa apakah model memiliki positional embedding.
