import os  # Utilitas pengelolaan path file.
import sys
import random  # Pengaturan seed untuk modul acak Python.
import argparse  # Parsing argumen baris perintah.
import numpy as np  # Operasi numerik berbasis array.
import pandas as pd  # Pembacaan dan pengolahan data tabular.
import torch  # Library utama PyTorch.
import torch.nn as nn  # Modul neural network dari PyTorch.
from torch.utils.data import DataLoader  # Loader untuk membentuk batch data.
from sklearn.model_selection import train_test_split  # Membagi data menjadi data latih dan validasi.
from transformers  import CLIPImageProcessor, CLIPVisionModelWithProjection, AutoTokenizer, AutoModel  # Komponen model dan tokenizer dari Hugging Face.
import wandb  # Logging eksperimen dan visualisasi training.

# Hindari deadlock tokenizer saat DataLoader memakai multiprocessing worker.
os.environ.setdefault('TOKENIZERS_PARALLELISM', 'false')

# Pastikan package di root project, seperti `models/`, tetap bisa diimpor saat dijalankan dari folder `src`.
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(CURRENT_DIR)
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

# Dukung eksekusi lewat `python src/main.py` maupun `python -m src.main`.
try:
    from .data_loader import MultimodalDataset  # Dataset untuk data multimodal (teks + gambar).
...
MODEL_NAME = 'mlB-image-textt'  # Nama model untuk membedakan hasil training dan file output.

class Config:
    # Path file dan direktori utama proyek.
    CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
    ...
    # Parameter utama untuk proses training.
    ...
    # Untuk `archA`: dimensi proyeksi fitur gambar.
    FUSION_IMAGE_DIM = 256
    # Untuk `archA`: dimensi proyeksi fitur teks.
    FUSION_TEXT_DIM = 256

    # Nama model pretrained yang dipakai.
...
def  main():
    ...
    # Atur seed agar hasil lebih konsisten saat eksperimen diulang.
    set_seed(config.SEED)
    os.makedirs(config.OUTPUT_DIR, exist_ok = True)
    
    # Simpan catatan training ke file lokal jika diberikan.
    if args.notes:
    ...
    # Inisialisasi W&B jika logging diaktifkan.
    ...
    if config.IMBALANCE_STRATEGY in {'class_weight', 'class_weight_focal'}:
        if args.class_weight_values is not None:
            ...
            # Hitung class weight seimbang dari data train.
    ...
    # `model.img_dim` adalah dimensi asli output CLIP,
    # sedangkan `model.fusion_img_dim` adalah dimensi target setelah projection/identity.
    ...
    # Gunakan Cross Entropy standar kecuali strategi imbalance diaktifkan.
    ...
    # Criterion cadangan; train/eval akan memprioritaskan `model.compute_loss` jika tersedia.
    ...
    # Susun nama file output berdasarkan nama model.
    ...
    for epoch in range(1, config.EPOCHS + 1):
        ...
        # Kirim metrik training dan validasi ke W&B.
        if use_wandb:
            wandb.log({
                'epoch': epoch,
        ...
        if improved:
            ...
            # Simpan checkpoint terbaik dengan nama file berbasis nama model.
            torch.save({
                'epoch': epoch,
            ...
            # Simpan ringkasan model terbaik ke W&B.
            if use_wandb:
    ...
    # Simpan plot evaluasi dengan nama file yang mengikuti nama model.
    ...
    if use_wandb:
        ...
        # Selalu unggah model terbaik sebagai artefak W&B saat logging aktif.
