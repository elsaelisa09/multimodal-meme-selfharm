import torch
import torch.nn as nn
import torch.nn.functional as F


class CLIPElectraFusion(nn.Module):

    def __init__(self, clip_model, electra_model,
                 fusion_img_dim=256,
                 fusion_text_dim=256,
                 num_classes=2, freeze_encoders=True):
        super().__init__()
        self.clip = clip_model
        self.electra = electra_model
        self.num_classes = num_classes
        self.fusion_img_dim = fusion_img_dim
        self.fusion_text_dim = fusion_text_dim
        self.focal_gamma = 2.0
        self.loss_strategy = 'none'
        self.class_weights = None

        if freeze_encoders:
            for p in self.clip.parameters():
                p.requires_grad = False
            for p in self.electra.parameters():
                p.requires_grad = False

        # Dimensi output fitur gambar dari CLIP.
        self.img_dim = clip_model.config.projection_dim  # Misalnya 512.

        # Dimensi output fitur teks dari ELECTRA.
        electra_hidden_dim = electra_model.config.hidden_size  # Misalnya 768.

        # Layer proyeksi teks: `electra_hidden_dim -> fusion_text_dim`.
        self.project_text = nn.Sequential(
            nn.Linear(electra_hidden_dim, fusion_text_dim),
            nn.GELU(),
            nn.LayerNorm(fusion_text_dim)
        )

        # Layer proyeksi gambar: `img_dim -> fusion_img_dim`.
        if fusion_img_dim == self.img_dim:
            self.project_image = nn.Identity()
        else:
            self.project_image = nn.Sequential(
                nn.Linear(self.img_dim, fusion_img_dim),
                nn.GELU(),
                nn.LayerNorm(fusion_img_dim)
            )

        # Dimensi hasil fusi adalah gabungan fitur gambar dan fitur teks.
        self.fusion_dim = fusion_img_dim + fusion_text_dim
        self.fusion_method = 'transformer'

        # Positional embedding untuk dua token: gambar dan teks.
        self.pos_embedding = nn.Parameter(torch.randn(1, 2, self.fusion_dim))

        # Transformer encoder dua lapis untuk memadukan kedua modalitas.
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=self.fusion_dim,
            nhead=8,
            dim_feedforward=self.fusion_dim * 4,
            dropout=0.1,
            batch_first=True
        )
        self.fusion_transformer = nn.TransformerEncoder(encoder_layer, num_layers=2)

        # Classifier MLP tiga lapis.
        self.classifier = nn.Sequential(
            nn.Linear(self.fusion_dim, self.fusion_dim // 2),  # Penyusutan dimensi tahap pertama.
            nn.GELU(),
            nn.Dropout(0.3),
            nn.Linear(self.fusion_dim // 2, self.fusion_dim // 4),  # Penyusutan dimensi tahap kedua.
            nn.GELU(),
            nn.Dropout(0.2),
            nn.Linear(self.fusion_dim // 4, num_classes)  # Proyeksi akhir ke jumlah kelas.
        )

    def forward(self, pixel_values, input_ids, attention_mask):

        # Ekstrak fitur gambar dari encoder visi CLIP.
        if hasattr(self.clip, 'get_image_features'):
            img_output = self.clip.get_image_features(pixel_values)
        else:
            img_output = self.clip(pixel_values=pixel_values)

        if hasattr(img_output, 'image_embeds'):
            img_feats = img_output.image_embeds
        elif hasattr(img_output, 'pooler_output'):
            img_feats = img_output.pooler_output
        elif isinstance(img_output, torch.Tensor):
            img_feats = img_output
        else:
            img_feats = img_output[0] if isinstance(img_output, (tuple, list)) else img_output
        
        # Normalisasi fitur gambar dengan L2 normalization.
        img_feats = img_feats / (img_feats.norm(dim=-1, keepdim=True) + 1e-10)

        # Proyeksikan fitur gambar setelah normalisasi.
        img_proj = self.project_image(img_feats)

        # Ekstrak fitur teks dari ELECTRA.
        txt_out = self.electra(input_ids=input_ids, attention_mask=attention_mask)
        last_hidden = txt_out.last_hidden_state
        # Bentuk tensor: `(batch_size, sequence_length, hidden_size)`.

        # Lakukan mean pooling pada token valid.
        # Attention mask memastikan token `[PAD]` tidak ikut dihitung.
        attn = attention_mask.unsqueeze(-1).float()  
        sum_emb = (last_hidden * attn).sum(dim=1)     
        sum_mask = attn.sum(dim=1).clamp(min=1e-9)    
        text_emb = sum_emb / sum_mask                
        # Bentuk tensor: `(batch_size, electra_hidden_dim)`.

        # Normalisasi embedding teks dengan L2 normalization.
        text_emb = text_emb / (text_emb.norm(dim=-1, keepdim=True) + 1e-10)

        # Proyeksikan embedding teks ke dimensi fusi.
        text_proj = self.project_text(text_emb)

        # Susun token multimodal secara eksplisit dalam layout `[text | image]`.
        # Token pertama memuat informasi gambar, token kedua memuat informasi teks.
        text_token = torch.cat([text_proj, torch.zeros_like(img_proj)], dim=-1)
        img_token = torch.cat([torch.zeros_like(text_proj), img_proj], dim=-1)

        # Urutan token yang masuk ke transformer adalah `[image, text]`.
        tokens = torch.stack([img_token, text_token], dim=1)
        tokens = tokens + self.pos_embedding

        # Lakukan fusi representasi dengan transformer encoder dua lapis.
        fused_tokens = self.fusion_transformer(tokens)

        # Gunakan token pertama sebagai representasi akhir untuk klasifikasi.
        fused_rep = fused_tokens[:, 0, :]

        # Teruskan representasi hasil fusi ke classifier.
        logits = self.classifier(fused_rep)

        return logits, img_proj, text_proj

    def configure_loss_strategy(self, strategy='none', class_weights=None, focal_gamma=None):
        """Atur strategi loss yang dipakai oleh `compute_loss` saat train/eval.

        Nilai yang didukung:
        - `none`: cross-entropy standar
        - `class_weight`: cross-entropy dengan bobot kelas
        - `focal`: focal loss
        - `class_weight_focal`: focal loss dengan bobot kelas
        """
        strategy = str(strategy).lower().strip()
        valid = {'none', 'class_weight', 'focal', 'class_weight_focal'}
        if strategy not in valid:
            raise ValueError(f"Unknown strategy '{strategy}'. Supported: {sorted(valid)}")

        self.loss_strategy = strategy
        if focal_gamma is not None:
            self.focal_gamma = float(focal_gamma)

        if class_weights is not None:
            self.class_weights = torch.as_tensor(class_weights, dtype=torch.float)
        elif 'class_weight' in strategy:
            raise ValueError("class_weights is required for class_weight-based strategies")
        else:
            self.class_weights = None

    def compute_loss(self, logits, labels):
        """Hitung loss sesuai strategi penanganan imbalance yang aktif."""
        strategy = self.loss_strategy
        gamma = self.focal_gamma

        if strategy == 'none':
            return F.cross_entropy(logits, labels)

        if strategy == 'class_weight':
            weight = self.class_weights.to(logits.device)
            return F.cross_entropy(logits, labels, weight=weight)

        if strategy == 'focal':
            ce = F.cross_entropy(logits, labels, reduction='none')
            pt = torch.exp(-ce)
            return (((1.0 - pt) ** gamma) * ce).mean()

        if strategy == 'class_weight_focal':
            weight = self.class_weights.to(logits.device)
            ce = F.cross_entropy(logits, labels, weight=weight, reduction='none')
            pt = torch.exp(-ce)
            return (((1.0 - pt) ** gamma) * ce).mean()

        raise RuntimeError(f"Unhandled strategy: {strategy}")


class EarlyStopping:
    
    def __init__(self, patience=3, mode='max'):
        self.patience = patience
        self.mode = mode
        self.best = None
        self.num_bad = 0
        self.should_stop = False
        
    def step(self, value):

        if self.best is None:
            self.best = value
            self.num_bad = 0
            return True
            
        improve = (value > self.best) if self.mode == 'max' else (value < self.best)
        
        if improve:
            self.best = value
            self.num_bad = 0
            return True
        else:
            self.num_bad += 1
            if self.num_bad >= self.patience:
                self.should_stop = True
            return False
